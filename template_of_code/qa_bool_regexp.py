...
def pre_processing(res_dict, paths):
    ''' Tout au long de cette fonction, on remplace des termes spécifiques par des noms de code
    et on applique des étapes de preprocessing de stemming et .lower'''
    df_list = []
    ...
    for path in paths:
        ...
        regexp = re.compile(
            '([^a-zA-Z]|^)(Data|DATA) (Processing|PROCESSING) (Agreement|Addendum|AGREEMENT|ADDENDUM)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' dpad ')
        df_temp.sentence = df_temp.sentence.str.lower()

        regexp = re.compile(
            '([^a-zA-Z]|^)cloud (service|services) provider(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' cloudserviceprovid ')
        regexp = re.compile(
            '([^a-zA-Z]|^)monitoring (service|services)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' monser ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(application|performance|deployment) monitoring(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' appmon ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(error|errors) tracking(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' errtrac ')
        regexp = re.compile('([^a-zA-Z]|^)marketing automation(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' marketautomation ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(business|product) analytics(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' businessanalytics ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(image|images) hosting(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' imagehosting ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(customer|customers) engagement(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' customerengagement ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(digital|certified) signature(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' digitalsignature ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(conversation|conversations) recording(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' conversationrecording ')
        ...
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' supportsystem ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(events|event) ticketing(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' eventticketing ')
        regexp = re.compile('([^a-zA-Z]|^)data visualization(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' datavisualization ')
        regexp = re.compile(
            '([^a-zA-Z]|^)intelligent search technology(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' intelligentsearchtechnology ')
        regexp = re.compile(
            '([^a-zA-Z]|^)intelligent analytics(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' intelligentanalytics ')
        regexp = re.compile(
            '([^a-zA-Z]|^)authentication (systems|system)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' authenticationsystems ')
        regexp = re.compile('([^a-zA-Z]|^)bill collection(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' billcollection ')
        regexp = re.compile(
            '([^a-zA-Z]|^)chat (services|service)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' chatservices ')
        regexp = re.compile(
            '([^a-zA-Z]|^)helpdesk (services|service)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' helpdeskservices ')
        regexp = re.compile(
            '([^a-zA-Z]|^)system administration (services|service)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' systemadministrationservices ')
        regexp = re.compile('([^a-zA-Z]|^)exception reporting(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' exceptionreporting ')
        regexp = re.compile(
            '([^a-zA-Z]|^)customer relationship management(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' customerrelationshipmanagement ')
        regexp = re.compile(
            '([^a-zA-Z]|^)(subscription|referral) management(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' subscriptionmanagement ')
        regexp = re.compile('([^a-zA-Z]|^)service provider(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' serviceprovider ')

        regexp = re.compile(
            '([^a-zA-Z]|^)data protection (law|laws)(?=[^a-zA-Z]|$)')
        ...
        regexp = re.compile(
            '([^a-zA-Z]|^)third[\- ]{0,1}(parties|party)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' thirdparti ')
        regexp = re.compile(
            '([^a-zA-Z]|^)sub[\- ]{0,1}(processor|processors)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' subprocessor ')
        regexp = re.compile(
            '([^a-zA-Z]|^)sub[\- ]{0,1}(contractor|contractors)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' subcontractor ')
        df_temp.sentence = df_temp.sentence.apply(remove_punct)

        regexp = '([^a-zA-Z]|^)contact us(?=[^a-zA-Z]|$)'
        df_temp.sentence = [sentence if not bool(
            re.search(regexp, sentence)) else '' for sentence in df_temp.sentence]

        regexp = re.compile(
            '([^a-zA-Z]|^)privacy (policy|policies)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' pp ')
        regexp = re.compile(
            '([^a-zA-Z]|^)standard contractual (clauses|clause)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' scc ')
        regexp = re.compile(
            '([^a-zA-Z]|^)binding corporate (rules|rule)(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' bcr ')

        regexp = re.compile(
            '([^a-zA-Z]|^)data processing (agreement|addendum)(?=[^a-zA-Z]|$)')
        ...
        df_temp.sentence = [' '.join(stem(sentence))
                            for sentence in df_temp.sentence]

        regexp = re.compile(
            '([^a-zA-Z]|^)(outsid (the|of the) (eu|eea|european union|european econom area))(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(regexp, ' outsideeu ')

        regexp = re.compile(
            '([^a-zA-Z]|^)(outsid ((your|of your) (countri|territori)|(the|of the) ((countri|territori) where you live)))(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' outsidecountri ')
        regexp = re.compile(
            '([^a-zA-Z]|^)countri other than the one in which you live(?=[^a-zA-Z]|$)')
        df_temp.sentence = df_temp.sentence.str.replace(
            regexp, ' outsidecountri ')
        regexp = re.compile(
            '([^a-zA-Z]|^)countri other than the one in which you live(?=[^a-zA-Z]|$)')
...
def get_countries(answer_list, df, n_countries, countries, question_id, nationalities=False, df_full=[]):
    ''' Renvoie les pays pour un document et une question par recherche de mots clés '''

    # If else avec présence de mots interdits
    if question_id in c.question_negative_key_words:
        # Recherche des réponses avec pays
    ...
    # Extraction des pays et nationalités
    ...
    # Conversion nationalité -> pays pour les questions 9-13
    ...
    if question_id in [18, 11, 9, 5] and len(country_extract) > 0:
        # On récupère la les pays de la
        # phrase avec le plus grand nombre de pays plus petit que la contrainte de nombre de pays de la réponse
        lengths = [len(set(countries)) for countries in country_extract if len(
            set(countries)) <= n_countries]
        if lengths:
            max_countries = max(lengths)
            answer_country = list(set([countries for countries in country_extract if len(
                set(countries)) == max_countries][0]))
            index = [i for i in range(len(country_extract)) if len(
                set(country_extract[i])) == max_countries][0]
        ...
        answer_country = list(
            set(itertools.chain.from_iterable(country_extract)))
        answer_country_justif = ''
    # conversion pays -> codes
...
def regexp_pred_countries(df_list, df_list_full_sentences):
    '''Renvoie les réponses pays pour tous les documents et toutes les questions'''
    res = [[] for i in range(len(df_list))]
    res_justifs = [[] for i in range(len(df_list))]
    # Boucle sur questions puis documents et appel de la fonction au dessus
...
def regexp_pred(df_list, df_list_full_sentences=None):
    '''Non utilisée voir fonction en dessous'''
...
def regexp_pred_justif(df_list, df_list_full_sentences=None):
    '''Réponses pour les questions booléennes'''
    question_key_words = c.question_key_words
    ...
    estimates = []
    answers_justif = []
    # Boucle sur les questions puis docs
    for question_id in question_key_words:
        answer_list = question_key_words[question_id]
        answers = []
        answers_justif_i = []
        ...
        for df_i in range(len(df_list)):
            ...
            for sentence_i in range(len(df.sentence)):
                ...
                if question_id in c.question_negative_key_words:
                    # Trouver une phrase qui match les mots clés définis pour la questions
                    resp = (any(all(any(key_word in sentence for key_word in key_word_list) for key_word_list in answer)
                                for answer in answer_list) and all(key_word not in sentence for key_word in c.question_negative_key_words[question_id]))
                else:
                    resp = any(all(any(key_word in sentence for key_word in key_word_list)
                                   for key_word_list in answer) for answer in answer_list)
                if resp:
                    break
            answers.append(resp)
            if question_id != 23:
    ...
    answers_justif.pop()
    return(estimates, question_key_words, np.array(answers_justif))


# =============================================================================
# Evaluation non utilisée pour le challenge
# =============================================================================
